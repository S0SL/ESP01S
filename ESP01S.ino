...
void button1_callback(const String & state)  //用state来存储组键的值按键 : "tap"(点按); "pre"(长按); "pup"(释放)开关 : "on"(打开); "off"(关闭)const String & state
{
  BLINKER_LOG("app操作了11!: ", state);       //串口打印
...
void setup()
{
  // 初始化串口
  Serial.begin(115200);
  wifi_station_set_hostname("ESPSSS");
  BLINKER_DEBUG.stream(Serial);
  BLINKER_DEBUG.debugAll();
  // 初始化有LED的IO
  pinMode(kg, OUTPUT);
  digitalWrite(kg, HIGH);              //默认关闭  LOW 时的是接通继电器，HIGH 时的是断开继电器
